            ...
            if (dbContext.Categories.Count() == 0)
            {
                ...
                foreach (var user in userManager.Users)
                {
                    ...
                    for (int j = 1; j < categories.Count; j++)
                    {
                        ...
                        var cards = cardDictionary[categories[j].Name];
                        var backs = getBacks(categories[j].Name);
                ...
                dbContext.Cards.AddRange(extraCards);
                dbContext.SaveChanges();
                ...
                foreach (var user in userManager.Users)
                {
                    ...
                    var backs = getBacks("Extra");
                    for (int i = 0; i < extraCards.Length; i++)
                    {
                        var back = backs[i];
                        back.LastModified = DateTime.Now;
                        back.CardId = extraCards[i].Id;
                        back.OwnerId = user.Id;
                        back.AuthorId = user.Id;
                    }
